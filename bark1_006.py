...
class App(ctk.CTk):
    ...
    def create_page(self, name):
        ...
        if name == "Home":
            ...
            self.create_reports_page(page_frame)
        elif name == "Playback":
    ...
    def generate_csv_report(self):
        """Generates a CSV report for recorded dog barking events, including timestamps and frequency analysis."""
        ...
        bark_events = []
        for timestamp in self.bark_timestamps:
            event_time = datetime.fromtimestamp(self.start_time + timestamp).strftime("%H:%M:%S")
            frequency = analyze_frequency(self.audio_buffer, SAMPLERATE)
            bark_events.append([event_time, f"{frequency:.2f} Hz"])

        header = ["Event Time", "Frequency (Hz)"]

        try:
            with open(csv_path, mode="w", newline="") as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(["Start Time", start_time_str])
                writer.writerow(["Elapsed Time", f"{int(elapsed_time // 3600):02}:{int((elapsed_time % 3600) // 60):02}:{int(elapsed_time % 60):02}"])
                writer.writerow(["Total Barks Detected", self.bark_count])
                writer.writerow(["Average Barks per Hour", f"{avg_barks_per_hour:.2f}"])
                writer.writerow([])
                writer.writerow(header)  
                writer.writerows(bark_events) 
    ...
    def generate_pdf_report(self):
        """Converts CSV Files from `/home/bark1/CSV_Files/` into properly formatted PDF_reports."""
        ...
        if not csv_files:
            print("? No CSV Files found in /home/bark1/CSV_Files Files/")
        ...
        tk.messagebox.showinfo("Success", "PDF_eports successfully generated in /home/bark1/PDF_Reports!")
    ...
    def save_bark_audio(self, pre_bark_audio):
        ...
        duration = 5  
        post_bark_audio = sd.rec(int(48000 * duration), samplerate=48000, channels=2, dtype='int16', device="hw:3,0")
        sd.wait()
